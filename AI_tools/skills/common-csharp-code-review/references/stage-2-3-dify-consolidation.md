...
- Known-fixed-schema JSON parsed with weak-typed access (`JsonElement.TryGetProperty`, `JObject`/`SelectToken` string-indexed chains, or `dynamic`) instead of a strong-typed DTO + `JsonSerializer.Deserialize<T>` / `JsonConvert.DeserializeObject<T>` — classify as `auto-fixable` when the schema is small/local (introduce the DTO classes and replace the access in place); classify as `needs-human` if the refactor would ripple across many call sites
