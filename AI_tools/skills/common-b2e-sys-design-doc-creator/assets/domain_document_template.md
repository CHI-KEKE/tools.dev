...
> 此表是 **Domain 核心業務概念的中英文索引**，方便依關鍵字反查所屬 Domain，也作為業務語意定義。
> 英文關鍵字寫的是「業務溝通會用到的英文名詞」，**不是程式碼 Entity 類別名**。
> 同步維護於 `.docs/B2E/domain-catalog.md`，請確保兩處一致。
>
> **收錄原則（嚴格遵守）：**
> - ✅ 列入：此 Domain 獨有或核心的業務名詞（其他 Domain 的人不看程式碼就搞不清楚的詞）
> - ❌ 排除：技術術語（Controller、Service、Repository、API、Worker 等）
> - ❌ 排除：通用業務詞（訂單、商品、用戶等在整個系統普遍存在、無需在此定義的詞）
> - ❌ 排除：程式碼類別名、DB 欄位名、設定值名稱
...
> ⚠️ 若圖中包含資料庫節點，連線指向的資料庫**必須**與下方「資料庫依據」表一致；同一 Domain 若涉及多個 DbContext（即多個實際資料庫），須各自畫出獨立節點，不可合併成一個 DB 節點。
...
| Application | 存取的 DB Table | 所屬 DbContext / 連線字串 | 依據 |
|---|---|---|---|
| [Application 名稱] | `table_name` | `XxxDbContext`（連線字串：`XxxConnection`） | `RepositoryClassName`（2A）/ 來自訪談（2B） |

> 圖中每條資料庫連線都必須能在此表找到對應列；若找不到依據，先回頭確認 Repository 實際使用的 DbContext，再修正圖中連線，不可憑 Table 名稱直接假設資料庫。
```
...
> 「所在 DB」須填寫實際的 DbContext 類別名稱 / connection string 名稱（來自 Repository 分析），不可用猜測或沿用其他 Table 的資料庫。

| Table 名稱 | 所在 DB（DbContext / 連線字串） | 主要用途 | 實作狀態 |
|---|---|---|---|
| `table_name` | `XxxDbContext`（`XxxConnection`） | 儲存... | ✅ 已實作 / 🔲 待實作 |
