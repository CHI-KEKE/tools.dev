...
3. **追蹤 Repository 層**：從 Service 找出存取的 Repository 與 DB Table，並記錄各 Repository 實際使用的 DbContext / connection string（避免多資料庫情境下誤判資料表所屬的資料庫）
...
| `01-c4-l1-system-context.md` | C4 L1 System Context：此 Domain 在整體系統中的位置，外部使用者與系統（**Mermaid graph**） | Q7（2B）/ code 掃描（2A） |
| `02-c4-l2-container.md` | C4 L2 Container：涉及哪些 Application、彼此串接方式，含資料庫連線（**Mermaid graph**，資料庫依據須對照實際 DbContext / connection string） | Q8（2B）/ code 掃描（2A） |
| `03-db-schema.md` | DB Table 清單 + table name + 實際所屬 DbContext / connection string；schema 欄位由 RD 補充 | Q11~Q12（2B）/ Repository 分析（2A） |
...
- [ ] `01-c4-l1-system-context.md`：**Mermaid graph** 呈現此 Domain 在整體系統中的位置，包含所有外部使用者與系統
- [ ] `02-c4-l2-container.md`：**Mermaid graph** 呈現 Application 串接關係；圖中若含資料庫連線，已對照 Repository 實際使用的 DbContext / connection string 確認無誤（2A 模式），不同 DbContext 的資料庫分開標示，不合併假設
- [ ] `03-db-schema.md`：包含 table name 與所在 DB（實際 DbContext / connection string 依據），schema 欄位有標示待補充方式
