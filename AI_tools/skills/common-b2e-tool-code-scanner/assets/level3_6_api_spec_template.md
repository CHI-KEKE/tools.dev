...
sequenceDiagram
  ...
  participant Service
  participant DB
  Client->>API: Request with Payload
  API->>Service: Validate and execute business logic
...
依據來源：`{ClassName}.{MethodName}`、`{relative/path/to/file.cs}`

---

### 2. 請求與回應 (Request & Response)
...
| 資料庫類型 | 資料表 / 集合 | 欄位 / Key | 型別 | 備註 |
|---|---|---|---|---|
| `{MSSQL/MongoDB/...}` | `{table_or_collection}` | `{column_or_key}` | `{Type}` | `{來源欄位或預設值}` |
| `{MSSQL/MongoDB/...}` | `{table_or_collection}` | `{column_or_key}` | `{Type}` | `{若未知填待確認}` |
